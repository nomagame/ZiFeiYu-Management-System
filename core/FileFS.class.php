...
class FileFS {
    ...
    function get_gpath($path, $type='u'){
        if ('r' == $type) $path = str_replace('//', '/', REAL_ROOT_PATH . $path);
        if ('u' == $type) $path = str_replace('//', '/', REAL_USER_PATH . $path);
        $path = SYS_WIN ? u2g($path) : $path;
        return $path;
    }
    function get_upath($path, $type='u'){
        $path = SYS_WIN ? g2u($path) : $path;
        if ('r' == $type) $path = substr($path, strlen(REAL_ROOT_PATH) -1);
        if ('u' == $type) $path = substr($path, strlen(REAL_USER_PATH) -1);
    ...
    public function rename($oldname, $newname){
        ...
        }else if(!is_writeable(get_dirname($oldname))){
    ...
    public function upload($path, $name, $cover=false){
        ...
        // Get parameters
        $chunk  = gpc('chunk', 'r','intval');
        $chunks = gpc('chunks','r','intval');
