...
$file = $datadir . '/ggcllist.php';
$content = file_get_contents( $file ); 
$file1 = $datadir . '/ggclrecyclelist.php';
$content1 = file_get_contents( $file1 );
if( strlen($content1) < 15 ){
	$content1= '<?php exit();?>'."\r\n";
}
foreach ($arraygid as $gid){
	if( strlen($gid) == strlen($clthread) + 15 ){
		$path = $datadir.'/'.$gid.'.js';
...
$conn->inoplog('删除广告策略', $gids, 1, getname());
die('1');
}
///////////////////////////
if( $menu == 'rebkggcl' ){
...
file_put_contents( $file , $content );
file_put_contents( $file1  ,$content1 );
$conn->inoplog('还原广告策略', $gids, 1, getname());
die('1');
}
///////////////////////////
if( $menu == 'delggcltrue' ){
...
$arraygid = explode(' ', $gids);
$file = $datadir.'/ggclrecyclelist.php';
$content = file_get_contents( $file ); 
foreach ($arraygid as $gid){
	if( strlen( $gid ) == strlen( $clthread ) + 15 ){
...
file_put_contents( $file , $content );
$conn->inoplog('彻底删除广告策略', $gids, 1, getname());
die('1');
}
///////////////////////////
?>