...
<head>
    ...
    <title>WebFTP v3.5 企业版 - Powered by OSDU.Net</title>
    ...
    <script type="text/javascript">
        var WF_HOST = '<?php echo $_SERVER['HTTP_HOST'];?>';
        var WF_PATH = '<?php echo ES_PATH;?>';
        var WF_NFSR = '<?php echo C('ROOT_PATH').C('USER_PATH');?>';
        var WF_FURI = 'http://' + (WF_HOST + WF_PATH + WF_NFSR).replace('/./', '/').replace(/([\/]+)/g, '/');
    </script>
</head>
<body id="body">
 ...
 <div class="col-main">
    <div id="list">
      ...
      <div id="list_main">
        ...
        <div id="list_main_center"><table class="tree-browser" cellpadding="0" cellspacing="0" ><tbody id="dirs-files-list"><th><font color="green">文件列表加载中...</font></th></tbody></table></div>
        <span id="list_main_right"></span><div class="clean"></div></div>
      <div id="list_foot"><span id="list_foot_left"></span><span id="list_foot_center"></span><span id="list_foot_right"></span><div class="clean"></div>
    </div>
    </div>
  </div>
...
<script id="tools-bar" type="text/template">
  <div class="apptools">
    <div class="clearfix apptools-inner">
      <a id="tool_select" class="btn" href="javascript:app.mui.SelectAll();" title="选择"><span><img src="static/images/toolbar/select.gif" width="16" height="16" />选择</span></a> <span class="edge">|</span>
      ...
      <a class="btn"  id="toolListView" href="javascript:void(0);" onclick="app.mui.olist=!app.mui.olist;app.nfs.nlist(app.nfs.cpath, false);" title="切换视图"><span><img src="static/images/toolbar/view_thumb.gif" width="16" height="16" />视图</span></a><span class="edge">|</span>
...
<script id="nlist-list-main" type="text/template">
    ...
    {@each list.files as file,idx}
        <tr class="file-list-${idx}">
            ...
            {@if file.ext=='jpg'}
                <td><a href="${path.WF_FURI}${file.path}" onclick="return false;" id="file-id-${idx}" data-path="${file.path}" data-name="${file.name}" data-ext="${file.ext}" data-mtime="${file.mtime}" data-chmod="${file.chmod}" title="双击预览图片" rel="show" colortitle="文件名称：<font color=red>${file.name}&nbsp;&nbsp;&nbsp;&nbsp;</font>图片大小: <font color=red>${file.fsize}</font>">${file.name}</a></td>
            {@else}
                <td><a href="${path.WF_FURI}${file.path}" id="file-id-${idx}" data-path="${file.path}" data-name="${file.name}" data-ext="${file.ext}" title="${file.name}" data-mtime="${file.mtime}" data-chmod="${file.chmod}" target="_blank" >${file.name}</a></td>
            {@/if}
            ...
            <td class="rhumbnail">
                ...
                {@each list.files as file,idx}
                    <div style="position:relative;left:0px;top:0px;">
                        {@if file.ext=='jpg'}
                            <ol class="f_icon rounded">
                                <a href="${path.WF_FURI}${file.path}" onclick="return false;" id="file-id-${idx}" data-path="${file.path}" data-name="${file.name}" data-chmod="${file.chmod}"  data-ext="${file.ext}" data-mtime="${file.mtime}" title="双击预览图片" rel="show" colortitle="文件名称：<font color=red>${file.name}&nbsp;&nbsp;&nbsp;&nbsp;</font>图片大小: <font color=red>${file.fsize}</font>">
                                    <div class="ext_big ext_big_${file.ext}"></div>
                                </a>
                            </ol>
                        {@else}
                            <ol class="f_icon rounded">
                                <a href="${path.WF_FURI}${file.path}" id="file-id-${idx}" data-path="${file.path}" data-name="${file.name}" data-chmod="${file.chmod}" data-ext="${file.ext}" title="${file.name}" target="_blank" >
                                    <div class="ext_big ext_big_${file.ext}"></div>
                                </a>
                            </ol>
                        {@/if}
...
<!-- colorBox 资源文件 -->
<link type="text/css" rel="stylesheet" href="static/plugins/colorBox/jquery.colorBox.css?v=1.3.17.2"/>
...
<script src="static/js/webftp2.nfs.js"></script>
<script src="static/js/webftp2.mui.js"></script>
<script type="text/javascript">
$(function () {
    // 配置参数
    // app.debug = true;
    app.api  = {
        ...
        url: 'webftps.php',
        auth:'<?php $auth = Session::get('uauth'); echo implode(',', $auth);?>'.split(',')
    };
